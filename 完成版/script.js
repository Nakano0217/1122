(() => {
  ...
  function fitCanvasToWindow() {
    // Resize canvas to match the CSS layout size but back it with device pixels
    // This prevents coordinate/scaling issues on high-DPI devices (iPad Retina)
    const dpr = window.devicePixelRatio || 1;
    const cssW = Math.max(1, Math.floor(window.innerWidth));
    const cssH = Math.max(1, Math.floor(window.innerHeight));

    // If size (CSS pixels) didn't change, skip
    const curCssW = parseInt(canvas.style.width || 0, 10);
    const curCssH = parseInt(canvas.style.height || 0, 10);
    if (curCssW === cssW && curCssH === cssH) return;

    // Set CSS size (what layout uses) and backing store size (physical pixels)
    canvas.style.width = cssW + 'px';
    canvas.style.height = cssH + 'px';
    canvas.width = Math.round(cssW * dpr);
    canvas.height = Math.round(cssH * dpr);

    // Scale drawing operations so 1 unit in canvas coordinate == 1 CSS pixel
